...
def score(key, prediction, verbose=False):
    ...
    if verbose:
        ...
        for relation in sorted(relations):
            ...
            sys.stdout.write("\n")
        print("")
        print("exclude examples which gold_label=Other and pred_label=Other")
        print("Precision (macro): {:.3%}".format(sum(precisions)/9))
        print("Recall (macro): {:.3%}".format(sum(recalls)/9))
        print("F1 (macro): {:.3%}".format(sum(f1s)/9))
        print("")
