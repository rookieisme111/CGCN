...
def parse_args():
    ...
    parser.add_argument('data_dir', help='Traffic dataset directory.')
    parser.add_argument('vocab_dir', help='Output vocab directory.')
    parser.add_argument('--sgns_dir', default='dataset/sgns', help='sgns directory.')
    parser.add_argument('--wv_file', default='sgns.target.word-character.char1-2.dynwin5.thr10.neg5.dim300.iter5', help='sgns vector file.')
    parser.add_argument('--wv_dim', type=int, default=300, help='vector dimension.')
...
def main():
    ...
    wv_file = args.sgns_dir + '/' + args.wv_file
    wv_dim = args.wv_dim
    ...
    # load sgns
    print("loading sgns...")
    sgns_vocab = vocab.load_sgns_vocab(wv_file, wv_dim)
    print("{} words loaded from sgns.".format(len(sgns_vocab)))
    
    print("building vocab...")
    all_tokens = train_tokens + dev_tokens + test_tokens
    v = build_vocab(all_tokens, sgns_vocab, args.min_freq)
    ...
    embedding = vocab.my_build_embedding(wv_file, v, wv_dim)
...
def build_vocab(tokens, sgns_vocab, min_freq):
    """ build vocab from tokens and sgns words. """
    ...
    if min_freq > 0:
        ...
        v = sorted([t for t in counter if t in sgns_vocab], key=counter.get, reverse=True)
